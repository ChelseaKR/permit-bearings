...
prototype in your browser, with no account. The pages use Google Analytics 4
to count visits (GPC and Do Not Track honored, an opt-out in every footer, ad
features off, addresses sent without query strings); project answers and
pasted text never reach it. See [the privacy page](https://chelseakr.github.io/permit-bearings/privacy.html) and
[ADR 0008](docs/adr/0008-google-analytics-4-on-the-public-pages.md).
...
make ai-eval                                        # live intake, grounding and ask evaluation; writes evals/ai/results/
...
An unrecognized field name or an unrecognized value is `2`, naming the field and
...
zone" is computable rather than a judgment call:
...
| Observability | N/A — the deployed artifact is a static, no-account showcase rather than a long-running production service. Its only telemetry is Google Analytics 4 page analytics on the production host (ADR 0008), which counts page views and never receives project answers or pasted text. The proposed no-storage beta runbook still requires host request-metadata and operational-system review because those records sit outside application telemetry. Storage, telemetry, uploads, or external model calls would trigger a new architecture and operational review; the optional AI service directed by ADR 0004 is exactly such a change and will need its own operational review before any hosted deployment. |
| Accessibility | Applies — Axe runs on all six public pages plus populated candidate-route and valid packet states; Lighthouse covers eight public page states. Browser tests also check 320px and 390px reflow, compact mobile navigation, the Spanish handoff language boundary, labeled mobile evidence records, document-level overflow, and the print summary's isolated print-media layout. The versioned human test matrix in `docs/MANUAL-VALIDATION.md` keeps physical-device, virtual-keyboard, keyboard, screen-reader, zoom, forced-colors, printed-output, and Spanish semantic review explicitly `not_run` until signed evidence exists. |
...
| AI Evaluation | Applies — the offline AI-assisted rule/explanation workflow has model-independent fixtures, and the runtime AI layer (ADR 0004) has a committed bilingual intake evaluation scored on per-field exact match and on abstention versus gap-filling, plus a citation-grounding evaluation counting claims whose quotes verify against the corpus, and a follow-up-answering set of 44 bilingual questions over the same eight confirmed-fact intakes, each labeled by what the offered passages can support — answerable (with the settling passage ids recorded), should-abstain (where the defect is answering with nothing handed to staff), or out of scope (zero tolerance on any claim shown) (`evals/ai/`, harness `python -m permit_pathways.ai.eval`). Results are committed only from recorded live runs that name provider, model, date, and commit; **the `ask` set has its scorer and its offline tests but no recorded run yet**, so no `/ask` number is published. Natural-language legal fidelity, applicant comprehension, and Spanish semantic parity remain unreviewed and are not inferred from any of these. |
...
| Data Governance | Applies — `docs/DATA-FLOW.md` records the no-storage boundary and every data path; `PROVENANCE.md`, `data/sources.json`, and `THIRD_PARTY_NOTICES.md` record source origin, retrieval dates, content hashes, and licensing; `docs/EXPORT-RESTORE.md` defines the deterministic public/synthetic evidence export and inert restore. The demo persists no applicant data. The schema.org markup on the public pages describes those pages and the browser tool and nothing else: no node describes `corpus/`, the mirrored statute and CEQA text, because a `Dataset` or `DataDownload` descriptor solicits dataset search engines and open-data catalogues to harvest an unofficial mirror of government documents as a published dataset, and a catalogue listing is far harder to withdraw than a page. CPRA, Information Practices Act, retention, and records-search behavior are designed but not executed or approved. |
...
- WCAG 2.2 AAA target with a static computed-contrast audit
  ...
  All six public pages load a locally maintained California Design System
...
applies to a given screen is a legal judgment this tool does not make. Three
