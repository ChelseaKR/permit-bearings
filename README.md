...
| Code Quality | Applies. Python 3.12 and development dependencies are locked; Ruff, mypy, branch coverage, generated-data parity, and 29 golden cases run through `make verify`. Ruff enforces complexity 10 and Bandit runs across `src`, `scripts`, and `demo`. Two scopes are narrower than that and are stated rather than implied: the 85% branch-coverage floor measures `src/permit_pathways` only, not the repository; and strict mypy is scoped to `src` (`scripts` reports 31 strict findings and `demo/app.py` 121, both open). `assets/demo.js` has no JavaScript unit runner or coverage threshold; it is exercised by the Python browser contract tests and the cross-runtime parity corpus, which `make verify` requires Node to run. |
