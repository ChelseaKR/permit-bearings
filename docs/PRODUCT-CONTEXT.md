...
| Versioned Woodland journey contract | Implemented data contract for prototype inputs | `data/journeys/woodland-preapproved-detached-adu.json` references one golden screening case and candidate route, the bounded readiness workflow and synthetic packet, and its applicant-editable applicability fact. `journey.py` resolves those references only when the route matches the complete fixture and is inside its source-review window on the sample's recorded evaluation date, scope and IDs agree, and readiness applicability is `applies`; the build emits a fingerprinted journey envelope containing the shared synthetic fact envelope with provenance, resolved route, route source-status clock, and complete readiness evidence. This is not a browser handoff, live parcel journey, eligibility finding, completeness assessment, or externally validated workflow: `check.html` and `prepare.html` remain separate surfaces and do not consume the envelope. |
...
7. **Browser and Python behavior can drift.** Screening, staleness, scanning,
   ...
   the Python implementation. The generated Woodland journey contract binds
   the Python route and readiness evidence at build time, but no browser
   route-to-packet transition consumes that contract yet.
...
current on the sample's evaluation date. This
makes route-to-packet agreement testable at build time, including explicit
workflow applicability and shared fingerprints. It does not make the browser
journey continuous or turn the synthetic records into a real applicant case.
