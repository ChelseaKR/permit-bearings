...
| Public/synthetic evidence export and restore | Implemented tooling for prototype data | `evidence_export.py`, its CLI, and `data/export/public-synthetic-evidence-v1.json` build one Git-HEAD-bound canonical ZIP from 58 exact public/synthetic files, verify raw hashes and a tree fingerprint, recheck normalized source-content hashes, preserve licensing/provenance and official-source gaps, enforce 17 pinned pending/`not_run` validation-state assertions, replay canonical loaders, and restore inertly into a new directory. Adversarial tests cover malformed profiles/manifests, unsafe or colliding members, compression/encryption/metadata drift, prefix/trailer smuggling, tampering, size limits, selected-file drift, destination refusal, cleanup, and deterministic bytes. The pinned allowlist excludes known sensitive material and rejects unlisted paths or asserted-state drift; it is not a privacy classifier. This is a prototype-scope evidence handoff, not applicant-data export, contractual ownership/offboarding, partner acceptance, authenticity/signing, backup/disaster recovery, CPRA search/export, completed review, or beta evidence. |
