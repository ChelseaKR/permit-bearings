...
| Transit proximity | Prototype | GTFS and statewide high-quality-transit data support screening in a CLI. Peak-window edge gaps, ferry-to-bus/rail connections, and the existing-versus-planned distinction are covered by regression tests: a statewide row marked `mpo_rtp_planned_major_stop` is reported on its own line and never counted toward the parking or height result, and the tool does not decide whether such a stop could qualify. Service effective dates/exceptions, multi-operator completeness, walking-network confirmation, and parcel integration still require correction before applicant-facing eligibility use. |
...
5. **Transit can overstate certainty.** Incomplete or single-operator feeds,
   ...
   conclusion. Statewide rows that exist only in a regional transportation
   plan are the settled case: they are excluded from both determinations and
   reported separately, because a screen that counts them states in the
   present tense that a facility exists when it does not.
