...
| Transit proximity | Prototype | GTFS and statewide high-quality-transit data support screening in a CLI. Peak-window edge gaps, ferry-to-bus/rail connections, and MPO-submitted planned stops are covered by regression tests. A row the Caltrans dataset marks `mpo_rtp_planned_major_stop` is reported by count, agency, type, and distance and never produces a candidate; ADR 0006 records why, and the module does not decide which statutory definition a standard incorporates. Walking distance over an operator-supplied offline pedestrian network is reported beside the straight line (`--pedestrian-network`; graph rules in ADR 0007), with `not_in_extract`, `snap_too_far` and `disconnected` withheld by name rather than filled in from the straight line; it changes no verdict, because which distance a jurisdiction applies is a judgement this module does not make. Multi-operator completeness and parcel integration still require correction before applicant-facing eligibility use. |
...
5. **Transit can overstate certainty.** Incomplete or single-operator feeds,
   ...
   outside the window, or the feed ships no calendar (issue #132).
   Walking-network confirmation is now available offline: `pedestrian.py`
   measures the walk over an operator-supplied OSM extract and
   `transit.py --pedestrian-network` prints it beside the straight line,
   under the graph rules ADR 0007 records. It changes no verdict, and a stop
   the extract cannot measure is named with its reason rather than given the
   straight-line figure. Multi-operator feed completeness still needs
...
6. **Local records are not applicant-ready layers.** The bounded Davis record
   ...
   2026-09-10 says **“Preapproved ADU List: Coming soon!”** The sample is a
