...
| Static browser delivery | Implemented surface for prototype data | Five task-focused pages use relative links and can run directly from disk or over HTTP. The applicant-first landing page loads no data JavaScript; the applicant, packet, review, and evidence pages load bundle format 4 from generated `data/demo-data.js` before shared page-gated application code. Canonical JSON remains authoritative; static tests and the build check fail when the bundle, generated readiness evidence, generated journey envelope, program-availability record, or rule-verification ledger drifts. The statewide receipt is rendered only from current page memory; the route-to-packet URL accepts exactly a journey ID and version and carries no project facts. |
