...
Status: 2026-08-04. This is the canonical product and claim context for the
...
| Rule verification levels | Prepared, not adopted | `src/permit_pathways/rule_verification.py` and `data/validation/rule-verification.json` add an explicit `machine_linked`/`human_reviewed`/`jurisdiction_approved` level on top of the bare `verified_on` date, per AGENTS.md's evidence rules. All 19 current rules are recorded `machine_linked`; none has an actual named reviewer or jurisdiction sign-off yet, and the ledger does not claim otherwise. A promoted level binds to the rule's exact citation fingerprint and a 180-day review window; `effective_status` fails a drifted or aged-out claim closed back to `machine_linked`. `tests/test_rule_verification.py` exercises schema validation, promotion, drift rejection, and time-based staleness, including on synthetic fixtures, since no real promotion exists to test against yet. The ledger never changes which rules match an intake. `python -m permit_pathways.harness` now prints a read-only summary of the effective level in force across the rule set; the ledger still has no browser or evidence-page surface. |
...
3. **Verification strength remains underspecified.** Explanation records bind
   ...
   ledger now exists for rules themselves (`rule_verification.py`), binds a
   promoted level to the rule's citation fingerprint and a 180-day window, and
   fails a drifted or aged-out claim closed. Every current rule is still only
   `machine_linked` — the ledger has no real promoted entry to point to and
   no independent signature. The harness CLI now prints a read-only count of
   the effective level in force, but there is still no browser or public
   evidence-page surface.
...
| P0 | Verification levels and evaluator provenance | Makes “verified” meaningful to staff and counsel. | Machine-linked, human-reviewed, and jurisdiction-approved states now exist for the full 19-rule set with tested promotion, drift-rejection, and staleness transitions (`rule_verification.py`); every rule is still only `machine_linked`. Next: recruit a named reviewer to actually promote one rule and wire the ledger into staff-facing display. |
