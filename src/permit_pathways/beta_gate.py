...
# These two bindings name artifacts the bundle builder regenerates. Unlike the
# planning ledgers, which are byte-pinned above, they legitimately change
# whenever the demo is rebuilt. A not-run gate therefore records the
# fingerprints it was prepared against rather than asserting that the working
# tree still holds those exact bytes. Freezing the lock, which records a
# commit_sha, is what turns those records into binding claims.
_REGENERATED_ARTIFACT_IDS = {"reference_journey", "reference_packet"}
...
def _load_bindings(
    ...
    for index, item in enumerate(rows):
        ...
        if actual_sha != expected_sha and artifact_id not in _REGENERATED_ARTIFACT_IDS:
...
def _reference_fields(
    ...
    *,
    ...
    skip = {"classification", "counts_as_active_pilot"}
    if not compare_fingerprints:
        skip |= {field for field in _REFERENCE_KEYS if field.endswith("_fingerprint")}
    for field in _REFERENCE_KEYS - skip:
...
def _validate_cross_bindings(
    ...
    # The journey and packet payloads are regenerated whenever the demo is
    # rebuilt, so in this not-run schema the reference records what the gate
    # was prepared against rather than tracking them. Only the identifiers,
    # which are stable across rebuilds, are cross-checked; the fingerprints
    # become binding when the lock is frozen against a commit.
    _reference_fields(
        reference,
        journey,
        "reference journey",
    ...
    # Source-status dates are derived from whenever the demo was last
    # evaluated, so a not-run answer key and the ledgers that share its lock
    # must agree with each other rather than with the live journey. Requiring
    # them to track the current evaluation date would invalidate prepared
    # human-review material every time the demo is rebuilt.
    _exact(
    ...
    # Same reasoning as the route dates above: the readiness evidence is
    # regenerated with the demo, so its evaluation-derived dates are not a
    # stable target for a prepared answer key.
    _iso_date_value(
