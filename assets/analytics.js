/*
 * Google Analytics 4 on the public Permit Bearings pages (ADR 0008).
 *
 ...
 *   /permit-bearings/, so local previews, the test servers on 127.0.0.1 and CI
 *   never send a hit to the real property;
 ...
 *
 * Project answers, pasted ordinance text and anything else typed into a page
 * are never read by this file and are never sent to Google.
 */
(function () {
  "use strict";

  // The one place the measurement ID goes. "" = no Google Analytics anywhere.
  // G-9FJBLEN2ZJ is the web stream of GA4 property 554878769 (provisioned
  // 2026-09-17 with 14-month retention and Google signals disabled).
  var GA4_ID = "G-9FJBLEN2ZJ";
  var PRODUCTION_HOST = "chelseakr.github.io";
  var PRODUCTION_PATH = "/permit-bearings/";
  // Renaming this key would silently opt every opted-out visitor back in.
  var OPT_OUT_KEY = "permit-bearings:analytics-opt-out";
  ...
  // The footer control. It is wired on every host, so the local accessibility
  // runs check the real, visible button, but it only ever changes the flag.
  function wireChoice() {
